...
def check_qubit(q, label=""):
    ...
    print("proba =", q.proba(round_values=True))
...
def check_qr(qr, label=""):
    ...
    print("proba =", qr.proba(round_values=True))
...
def test_qubits():
    ...
    check_qubit(q_plus, "q+ (|+>)")  # attendu proba {0:0.5, 1:0.5}

    q_phase = gates.apply_gate_single(gates.Rphi(np.pi/2), q_plus)
    check_qubit(q_phase, "q+ après Rphi(pi/2)")
    # attendu: |ψ> = 0.71|0> + 0.71j|1>
...
def test_single_gates():
    ...
    q = gates.apply_gate_single(gates.X, q)  # doit donner |1>
    check_qubit(q, "après X")

    q = gates.apply_gate_single(gates.H, q)  # H|1> = (|0>-|1>)/√2
    check_qubit(q, "après H")
    # attendu proba {0:0.5, 1:0.5}
...
def test_quantum_register():
    ...
    check_qr(qr, "init QR (|00>)")
    # attendu [1,0,0,0]

    qr = gates.apply_gate_multi(gates.H, qr, target_qubit=0)
    check_qr(qr, "après H sur q0")
    # attendu (|00>+|10>)/√2 → [0.5, 0, 0.5, 0]

    qr = gates.apply_gate_multi(gates.H, qr, target_qubit=1)
    check_qr(qr, "après H sur q1")
    # attendu |++> → [0.25,0.25,0.25,0.25]

    qr = gates.apply_gate_double(gates.CNOT, qr)
    check_qr(qr, "après CNOT")
    # attendu état de Bell (|00>+|11>)/√2 → [0.5,0,0,0.5]
...
def test_three_qubits():
    ...
    check_qr(qr, "init QR (|0>|+>|1>)")
    # attendu (|001>+|011>)/√2 → proba [0,0.5,0,0.5,0,0,0,0]

    qr = gates.apply_gate_multi(gates.H, qr, target_qubit=0)
    check_qr(qr, "après H sur q0")
    # attendu (|001>+|011>+|101>+|111>)/2 → [0,0.25,0,0.25,0,0.25,0,0.25]
